...
A terminal-based treasure hunting game built in Python. Navigate a 16x16 grid, collect treasures, avoid deadly snakes and tigers, and survive within a 2-minute time limit — all from your command line.

## Narrative

You are **Australian Jones**, a daring treasure hunter on a mission to recover lost relics scattered across a dangerous, wall-filled ruin. Deadly 🐍 snakes and fierce 🐯 tigers roam the corridors, slowly creeping toward you. You have a limited supply of ammo and only two lives. Collect 10 treasures before time runs out, or the ruins claim another victim.
...
### Hazard AI
- 3–5 hazards spawn on the grid as either 🐍 snakes or 🐯 tigers, randomly assigned
- Hazards chase you using greedy pathfinding and respect walls, trying alternate routes when blocked
...
The test suite includes 113 tests covering movement, walls, shooting, hazard AI, hazard types, spawning, collisions, respawn delays, win/lose conditions, and theme consistency.
...
ai-terminal-game/
  ...
  test_game.py    # pytest test suite (113 tests)
  README.md       # This file
```
...
Writing 113 tests meant that every change could be verified in under a second. Tests caught regressions immediately — a wrong emoji constant, a broken wall check, or a hazard that could walk through walls. The discipline of running `pytest` before every commit ensured that only working code reached GitHub.
